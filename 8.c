		...
		switch(c)
		{
			...
			case '-':
				...
				st[i]='\0';
				 ...
				 while(++count<=1008) 
				 {
				 	if(b&&rsl[count]==0&&count!=1008) {printf(" ");continue;}
				 	printf("%c",num[rsl[count]]);rsl[count]=0;
				 	b=0;
				 }
				 if(b) printf("0");printf("\n\n");
				 break;		
			case '*':
				temp=x;
				while((ch=st[y--])!='*')
				...
				b1=b=(1008-noz)>iml?(1008-noz):iml;
				st[i]='\0';
				 printf("%*s\n",b,st);
				 st[i]='*';
				 printf("%*s\n",b,st+i);
				 if(m==1) while(b1--)printf("-");
				 else if(iml>b)
				 while(iml--)printf("-");
				 else {b1=b-iml;while(b1--) printf(" ");while(iml--) printf("-");}
				 printf("\n");
				 b1=b;
